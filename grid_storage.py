...
class GridStorage:
    def __init__(self, world_dimensions: Vector, origin: Vector, dsep):
        ...
        self.grid = []
        for x in range(0, self.grid_dimensions.x):
            self.grid.append([])
            for y in range(0, self.grid_dimensions.y):
                self.grid[x].append([])
    ...
    def add_sample(self, v, coords=None):
        ...
        self.grid[coords.x][coords.y].append(v)
        ...
        for x in range(-1, 2):
            for y in range(-1, 2):
                ...
                if not self.vector_out_of_bounds(cell, self.grid_dimensions):
                    if not self.vector_far_from_vectors(v, self.grid[cell.x][cell.y], d_sq):
                        return False
        return True
    ...
    # called every integration step
    def get_sample_coords(self, world_v: Vector):
